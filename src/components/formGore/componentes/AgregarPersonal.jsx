...
export const AgregarPersonal = ({
  ...
  const arregloCalidadJuridica = Object.entries(personas).map(
  ...
  useEffect(() => {
    const esquema = validacionesCalidadJuridica(arregloCalidadJuridica);
  ...
  const agregarPersona = async (calidadJuridicaLabel) => {
    ...
    const calidadJuridicaObjeto = listado_calidades_juridicas.find(
      (cj) => cj.calidad_juridica === calidadJuridicaLabel
    );
    ...
    try {
      const response = await updatePasoGore(payload);
      ...
      if (response && response.data.p_5_3_a_personal_directo) {
        ...
        setPersonas((prevPersonas) => ({
          ...prevPersonas,
          [calidadJuridicaLabel]: [
            ...(prevPersonas[calidadJuridicaLabel] || []),
            nuevaPersona,
          ],
        }));
      } else {
        console.error(
          ...
          response ? response.message : 'Respuesta vacía'
        );
      }
    } catch (error) {
  ...
  const eliminarPersona = async (persona, idFila) => {
    ...
    try {
      ...
      setPersonas((prevPersonas) => {
        const filasActualizadas = prevPersonas[persona].filter(
          (fila) => fila.id !== idFila
        );
  ...
  const updateFieldState = (personaId, fieldName, newState) => {
    setPersonas((prevPersonas) => {
      ...
      Object.keys(nuevasPersonas).forEach((calidadJuridica) => {
        ...
        const index = nuevasPersonas[calidadJuridica].findIndex(
  ...
  const handleInputChange = (personaId, campo, valor) => {
    setPersonas((prevPersonas) => {
      const nuevasPersonas = { ...prevPersonas };
      Object.keys(nuevasPersonas).forEach((calidadJuridica) => {
        nuevasPersonas[calidadJuridica] = nuevasPersonas[calidadJuridica].map(
          (persona) => {
  ...
  useEffect(() => {
    if (nuevaCalidadJuridica) {
      const ejecutarAgregarNuevaCalidadJuridica = async () => {
        await agregarNuevaCalidadJuridica(
          nuevaCalidadJuridica.value,
          nuevaCalidadJuridica.label
        );
      };
  ...
  const agregarNuevaCalidadJuridica = async (
    calidadJuridicaSeleccionada,
    labelSeleccionado
  ) => {
    const payload = {
      [seccion]: [
    ...
    try {
      ...
      if (response && response.data.p_5_3_a_personal_directo) {
        const listaActualizadaPersonalDirecto =
          response.data.p_5_3_a_personal_directo;

        const nuevaCalidadJuridicaDatos = {
          ...listaActualizadaPersonalDirecto[
            listaActualizadaPersonalDirecto.length - 1
          ], // Extrayendo el último elemento
        };
        ...
        setPersonas((prevPersonas) => {
          ...
          nuevasPersonas[labelSeleccionado] =
            nuevasPersonas[labelSeleccionado] || [];
  ...
  useEffect(() => {
    ...
    const opcionesFiltradas = listado_calidades_juridicas.filter(
      (calidad) => !calidadesEnUso.includes(calidad.calidad_juridica)
    );

    // Actualizar opcionesCalidadJuridica con las opciones filtradas
    const opcionesActualizadas = transformarEnOpciones(
      opcionesFiltradas,
      'calidad_juridica'
    );
  ...
  const handleSave = async (arrayNameId, fieldName, newValue) => {
    ...
    if (fieldName === 'calidad_juridica') {
      ...
      if (fieldName === 'estamento') {
        ...
        payload = {
          [seccion]: [
            { id: arrayNameId, [fieldName]: personaEncontrada[fieldName] },
          ],
        };
      }
    }
    try {
      ...
      updateFieldState(arrayNameId, fieldName, {
        loading: false,
        saved: false,
      });
    }
  ...
  return (
    <div className="my-4">
      <div className="col my-4">
        <form onSubmit={handleSubmit(onSubmitAgregarPersona)}>
          {Object.entries(personas).map(
            ([calidad_juridica, personas], index) => (
              <div key={index}>
                <div>
                  <span className="text-sans-p-bold mt-4">
                    Calidad Jurídica:
                  </span>
                ...
                {personas.map((persona, personaIndex) => (
                  <div
                    key={persona.id}
                    className={`row py-3 ${
                      personaIndex % 2 === 0 ? 'white-line' : 'neutral-line'
                    } align-items-center me-3`}
                  >
                    <div className="col-1">
                      {' '}
                      <p className="text-sans-p-bold mt-3">
                        {personaIndex + 1}
                      </p>{' '}
                    </div>
                    <div className="col">
                        ...
                        render={({ field }) => {
                          ...
                          return (
                            <InputCosto
                              ...
                              onBlur={handleBlur}
                              loading={
                                persona.estados?.renta_bruta?.loading ?? false
                              }
                              saved={
                                persona.estados?.renta_bruta?.saved ?? false
                              }
                              error={
                                errors[`renta_bruta_${persona.id}`]?.message
                              }
                    ...
                    {!solo_lectura && (
                      <div className="col">
                        <button
                          ...
                          onClick={() =>
                            eliminarPersona(calidad_juridica, persona.id)
                          }
                        >
                ...
                {!solo_lectura && (
                  ...
                  >
                    ...
                    <p className="mb-0 text-decoration-underline">
                      Agregar {personas[0]?.nombre_calidad_juridica}
                    </p>
                  </button>
                )}

                {personalGore.map((item, itemIndex) => {
                  const itemCorrespondiente = Object.entries(
                  ...
                  if (itemCorrespondiente) {
                    return (
                      <div key={itemIndex} className="my-4">
                        <p className="text-sans-p-bold">
                          Resumen de justificación de costos de personal
                          directo: {item.label}
                        </p>
                        <h6 className="text-sans-h6-primary mt-3">
                          Debes justificar el 100% del costo informado en el
                          punto 5.1a para completar esta sección.
                        </h6>
                          ...
                          <table className="table table-secondary my-4">
                            <thead>
                              <tr>
                                <th scope="col">
                                  <span className="py-2">
                                    Costos Indirectos <br /> Totales Anual ($M)
                                  </span>
                                </th>
                                <th scope="col">
                                  <span className="py-2">
                            ...
                            <tbody>
                              <tr
                                className="table-light text-center py-5 "
                                key={item.id}
                              >
                                <td></td>
                                <td className="py-3">
      ...
      {mostrarBotonFormulario && !solo_lectura && (
        <button className="btn-secundario-s m-2" onClick={mostrarFormulario}>
          <i className="material-symbols-rounded me-2">add</i>
          <p className="mb-0 text-decoration-underline">
          ...
          render={({ field }) => {
            ...
            return (
              <CustomTextarea
                ...
                onBlur={handleBlur}
                loading={
                  paso5.descripcion_funciones_personal_directo.estados
                    ?.descripcion?.loading ?? false
                }
                saved={
                  paso5.descripcion_funciones_personal_directo.estados
                    ?.descripcion?.saved ?? false
                }
                error={
                  errors[
                    `descripcion_${paso5.descripcion_funciones_personal_directo.id}`
                  ]?.message
                }
                readOnly={solo_lectura}
              />
