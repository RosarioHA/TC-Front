      ...
      {Object.keys(selectedOptions).map((tipoUsuario) => (
        <div key={tipoUsuario} className="mb-5 mt-5">
          ...
          <table>
            ...
            <tbody>
            {selectedOptions[tipoUsuario] && selectedOptions[tipoUsuario].map((selectedOption, index) => {
                const usuarioNombre = selectedOption ? selectedOption.nombre : `Competencia ${selectedOption.id}`;

                return (
                  <tr key={selectedOption.id} className={index % 2 === 0 ? 'neutral-line' : 'white-line'}>
                    <td> <p className="ms-4 my-3">{index + 1}</p> </td>
                    <td> <p className="my-3">{usuarioNombre}</p> </td>
                    <td>
                      <button className="btn-terciario-ghost" onClick={() => handleCheckboxChange(selectedOption)}>
